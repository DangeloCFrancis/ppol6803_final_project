...
:::{.callout-important title="Imputation Methods"}

`tidyrecipes` provides multiple functions to 'impute' or fill in the missing value(s) of observations, usually labeled as `step_impute_<method>`. Functions such as `step_impute_mean()` and `step_impute_roll()` are computationally inexpensive. However, these functions are not fit for multinomial, or multi-class, predictive models.

We decided to use the algorithmic imputation function `step_impute_bagged()`, since it is better suited for multinomial predictive models compared to a linear imputation and *k*-nearest neighbor approach.
...
**ROC-AUC**
: Stands for *R*eceiver-*o*perating *c*haracteristic_*A*rea *u*nder the *c*urve. This maps the true positive rate of our model. The closer the value is to 1, the better the model is at predicting events.
:::
...
Surprisingly, our random forests model did not perform well in predicting the political identification of South Korean voters. Despite this, South Korean citizens have recently demonstrated their democratic resilience by successfully thwarting an attempted coup by the sitting president, Yoon Suk Yeol[^9], preventing a regression to authoritarian rule—a resilience rooted in their historical struggles against autocratic leaders. In stark contrast, mainland China has seen an erosion of the few remaining democratic norms. In 2018, President Xi Jinping abolished term limits in 2018's constitutional amendment[^10]. Under Xi, China claims to have the 'broadest, truest, and most effective democracy' through its concept of 'whole-process people's democracy'[^11]. However, in recent years, China has become increasingly repressive as the CCP tightens control over all aspects of life and governance. In 2023, China was ranked as the third least electorally democratic country in the world. This raises a critical question: Do the determinants of political preferences in an entirely different political regime, align with those in democratic countries? 

[^10]: Ng, K., Kwon, J., Ku, Yuna.; [South Korea opposition files motion to impeach President Yoon](https://www.bbc.com/news/articles/c8rjd064012o), 4 December 2024

[^11]: Doubek, J.; [China Removes Presidential Term Limits, Enabling Xi Jinping To Rule Indefinitely](https://www.npr.org/sections/thetwo-way/2018/03/11/592694991/china-removes-presidential-term-limits-enabling-xi-jinping-to-rule-indefinitely), 11 March 2018

[^12]: Chinese People's Daily; [The broadest, truest, and most effective democracy — as guided by General Secretary Xi Jinping in advancing whole-process people's democracy](https://www.gov.cn/xinwen/2023-03/03/content_5744265.htm), 3 March 2023

### Asian Barometer Survey Data 
We use data from the Round 5 Asian Barometer Survey, conducted between 2018 and 2021, just after the constitutional amendment. From the original 306 variables, we select 156 variables that primarily cover information on thermometer ratings and demographics. Most of the dropped variables were excluded due to a high volume of missing data, likely resulting from censorship of speech. Our outcome variable is **q104**: *Where would you place our country [China] today? [Is] democracy suitable or not? [1:10][Democracy is completely unsuitable:Democracy is perfectly suitable]*
