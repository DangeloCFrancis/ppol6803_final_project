...
We were surprised to see questions **VCF0830**—*Aid to Blacks Scale*— appear on the variable importance graph. Most surprising is that question **VCF9239**-*How important is gun control issue to respondent*— is at the very bottom below the aforementioned questions. To investigate this further, we ran a regression to test for potential multicollinearity. The variance inflation factors (VIF) from this regression indicate that there is no multicollinearity between VCF9239 and the other top 19 variables. Is this a case of overestimating the effect of gun control on voter political ID, specifically in the media? 
...
### Asian Barometer (mainland China)
...
china_vip_plot +
  labs(
    title = str_wrap("Variable importance in determining mainland China's political institution preference"),
    subtitle = "",
    caption = "")
...
## Testing China predictive model 
